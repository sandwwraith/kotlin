...
class UseExpressionBodyInspection(private val convertEmptyToUnit: Boolean) : AbstractKotlinInspection() {
    ...
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean) =
            object : KtVisitorVoid() {
                override fun visitDeclaration(declaration: KtDeclaration) {
                    ...
                    val (value, suffix, highlightType) = statusFor(declaration) ?: return

                    val problemDescriptor = holder.manager.createProblemDescriptor(
                            declaration,
                            value?.textRange?.shiftRight(-declaration.startOffset),
    ...
    private fun KtExpression.toHighlight() = when (this) {
